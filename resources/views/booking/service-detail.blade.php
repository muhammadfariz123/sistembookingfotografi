...
<body class="bg-gray-50 min-h-screen text-gray-800">
    ...
    <main class="max-w-6xl mx-auto py-10 px-4">
        ...
        <div class="flex flex-col lg:flex-row gap-8 items-start">
            ...
            {{-- KOLOM KIRI (Info Paket & Galeri) --}}
            <div class="w-full lg:w-2/3 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">

                {{-- Label & Judul --}}
                <div class="mb-6">
                    <p class="text-brand font-bold text-xs tracking-widest uppercase mb-2">PAKET FOTO</p>
                ...
                {{-- Keuntungan Booking (Kotak Abu-Abu) --}}
                ...
                {{-- Galeri Foto --}}
                <div>
                    <h3 class="font-bold text-gray-900 mb-4 text-lg border-b border-gray-200 pb-2">Contoh Hasil Foto
                    </h3>
                    {{-- Grid Foto Asli dari Database --}}
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        @php 
                                    // Mengambil galeri yang terkait dengan paket ini
                 ...
                 @if($totalPhotos > 0)
                     ...
                     @foreach($galleries->take(6) as $index => $gallery)
                          ...
                          @if($index == 5 && $totalPhotos > 6)
                               {{-- Kot     ak Terakhir (Ke-6): Overlay "Lihat Selengkapnya" --}}
        ...
        <p class="text-gray-400 text-sm">Belum ada foto galeri untuk paket ini.</p>
                        </div>
    @endif
                </div>

                                       
                {{-- Tombol Lihat Selengkapnya (Terpisah di Bawah Grid) --}}
                    @if($totalPhotos > 6)
            ...
            {{-- KOLOM KANAN (Kotak Harga & Tombol Aksi - Sticky) --}}
            <div class="w-full lg:w-1/3 sticky top-24">
                <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">

                    {{-- Harga dengan Highlight --}}
                    <div class="mb-6">
                        <div class="inline-block bg-amber-100 text-brand font-bold text-xs px-3 py-1 rounded-full mb-3">
                            HARGA PAKET
                        </div>
                        <p class="text-4xl font-extrabold text-gray-900 tracking-tight">Rp
    ...
    Booking Sekarang
        ...
        <div class="mt-12 text-center lg:text-left">
            <a href="{{ route('booking.services.all', $ownerId) }}"
                class="inline-flex items-center justify-center gap-2 border-2 border-gray-300 text-gray-700 px-8 py-3 rounded-xl text-sm font-bold hover:bg-gray-50 transition">
