{{-- resources/views/booking/public.blade.php --}}
<!DOCTYPE html>
<html lang="id" class="scroll-smooth">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ $companySetting?->company_name ?: ($owner->name ?? 'Nama Usaha Anda') }} — Studio Foto Profesional</title>
...
<body class="bg-white text-gray-800 antialiased">
    ...
    <div class="bg-brand text-white text-center py-24 px-4">
        <div class="max-w-3xl mx-auto">
            <h1 class="text-4xl md:text-[52px] font-bold mb-6 leading-tight">Abadikan Momen<br>Terbaikmu</h1>
    ...
    <div id="paket" class="bg-gray-50 py-20 px-4 md:px-12" x-data="{ activeCategory: 'Semua' }">
        <div class="text-center max-w-2xl mx-auto mb-10">
            ...
            <p class="text-gray-500 text-sm md:text-base">Temukan paket yang paling sesuai dengan kebutuhan dan momen spesialmu.</p>
        </div>

        @if($services->isEmpty())
            <div class="text-center py-10 text-gray-400 border border-dashed border-gray-300 rounded-xl max-w-3xl mx-auto bg-white">
                Belum ada paket layanan yang ditambahkan oleh admin.
            </div>
        @else
            @php
                // MENGAMBIL NAMA KATEGORI DARI RELASI BARU
                $categories = $services->map(function($item) {
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                @foreach($services as $service)
                    ...
                    <div x-show="activeCategory === 'Semua' || activeCategory === '{{ $catName }}'" x-transition.opacity 
                        class="border border-gray-200 rounded-[20px] p-6 sm:p-8 hover:shadow-lg transition duration-300 bg-white flex flex-col h-full relative overflow-hidden">
                        
                        <div class="flex-grow">
                            <span class="inline-block bg-orange-50 text-brand text-[10px] font-bold px-2.5 py-1 rounded-md mb-3 uppercase tracking-wider">
                                {{ $catName }}
                            </span>
                            <h3 class="text-xl font-bold text-gray-900 mb-3">{{ $service->name }}</h3>
                            @if($service->description)
                                <p class="text-sm text-gray-500 mb-4 whitespace-pre-line line-clamp-3">{{ $service->description }}</p>
                            @endif
                        </div>

                        <div class="mt-auto pt-4 border-t border-dashed border-gray-100">
                            <h4 class="text-2xl font-bold text-brand mb-6">Rp {{ number_format($service->price, 0, ',', '.') }}</h4>
                            <div class="flex flex-col gap-3">
                                <a href="{{ route('booking.service.detail', ['ownerId' => $ownerId, 'serviceId' => $service->id]) }}" class="w-full py-3 text-center rounded-xl border border-gray-200 text-gray-700 font-semibold text-sm hover:bg-gray-50 transition">
        ...
        <div class="pt-16 pb-12 text-center flex flex-col items-center justify-center">
            ...
            <a href="https://wa.me/{{ $companySetting?->whatsapp_number ?? '6281234567890' }}" target="_blank"
