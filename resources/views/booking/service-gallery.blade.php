...
<head>
    ...
    <title>Galeri Foto {{ $service->name }} — {{ $companySetting?->company_name ?? $owner->name }}</title>
...
<body class="bg-gray-50 text-gray-800 antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-grow max-w-6xl mx-auto w-full py-10 px-4">
        ...
        <div class="mb-10">
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight mb-3">{{ $service->name }}</h1>
            <p class="text-gray-500 text-sm md:text-base">
                Berikut adalah keseluruhan portofolio dan hasil foto untuk paket ini. Klik pada gambar untuk memperbesar.
            </p>
        </div>

        {{-- GRID FULL GALLERY --}}
        @if($service->galleries->isEmpty())
            <div class="text-center py-16 bg-white border border-dashed border-gray-300 rounded-2xl">
                <p class="text-gray-500">Belum ada foto galeri untuk paket ini.</p>
            </div>
        @else
