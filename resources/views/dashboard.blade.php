                ...
                <template x-for="item in bookingCards" :key="item.key">
                    <button @click="setStatus(item.key)"
                        ...
                        <div class="text-left min-w-0">
                            ...
                            <!-- Angka dari summary backend -->
                            <h1 class="text-[28px] font-bold mt-2" :class="item.text"
                                x-text="summary[item.key] ?? 0"></h1>
                        </div>
    ...
    <script>
        ...
        function bookingForm() {
            return {
                ...
                openEditBooking(booking) {
                    ...
                    this.bookingTime = booking.booking_time ? String(booking.booking_time).substring(0, 5) : ''
                    ...
                    if (discPct > 0) {
                        ...
                        this.$nextTick(() => { if (this.$refs.discountInput) this.$refs.discountInput.value = discPct })
                    } else {
                        this.discountType  = 'rupiah'
                        this.discountValue = 0
                        this.$nextTick(() => { if (this.$refs.discountInput) this.$refs.discountInput.value = 'Rp 0' })
                    }
                    ...
                    const parentEl = document.querySelector('[x-data^="dashboardFilter"]') ?? document.querySelector('[x-data]')
                    if (parentEl) {
                        const parent = Alpine.$data(parentEl)
                        if (parent && typeof parent.openBookingModal === 'function') parent.openBookingModal()
                    }
                },
                ...
                resetToCreate() {
                    ...
                    this.quantity         = 1
                    this.unitPrice        = this.services.length > 0 ? parseInt(this.services[0].price) || 0 : 0
                    this.paidAmount       = 0
                    this.discountValue    = 0
                    this.discountType     = 'rupiah'
                    this.notes            = ''
                ...
                async deleteService(service) {
                    ...
                    try {
                        const res = await fetch('/service-types/' + service.id, { method: 'DELETE', headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content } })
                        const result = await res.json()
                ...
                get discountAmount() {
                    if (this.discountType === 'percent') return Math.round(this.subtotal * (this.discountValue / 100))
                ...
                async submitBooking() {
                    ...
                    const payload = {
                        ...
                        discount_percent: this.discountPercentForBackend, paid_amount: this.paidAmount, notes: this.notes,
                    }
                    try {
                        ...
                        const parentEl = document.querySelector('[x-data^="dashboardFilter"]') ?? document.querySelector('[x-data]')
                        if (parentEl) { const parent = Alpine.$data(parentEl); if (parent && typeof parent.closeBookingModal === 'function') parent.closeBookingModal() }
                        this.resetToCreate()
                    ...
                    } catch (err) { Swal.fire({ icon: 'error', title: 'Gagal!', text: 'Gagal terhubung ke server.', confirmButtonColor: '#2563eb', customClass: { popup: 'rounded-[28px]' } }) }
        ...
        function dashboardFilter() {
            return {
                ...
                // ── Init: load summary sekaligus saat halaman dibuka ───
                init() {
                    this.$nextTick(() => { if (window.lucide) lucide.createIcons() })
                    this.loadSummary()
                    // Watcher filter
                    ...
                    // Reload summary juga saat tabel reload
                    window.addEventListener('reload-bookings', () => this.loadSummary())
                },

                // ── Fetch summary dari /bookings ───────────────────────
                async loadSummary() {
                    try {
                        ...
                        // Biarkan angka tetap 0 jika gagal
                    }
                },
