...
{{-- STORE GLOBAL UNTUK STATUS SIDEBAR (collapsed / mobile open) --}}
{{-- Status "collapsed" disimpan di localStorage supaya konsisten walau pindah halaman (full page reload). --}}
<script>
...
<div x-show="$store.sidebar.mobileOpen" x-transition.opacity @click="$store.sidebar.mobileOpen = false"
...
<aside x-data="{ isDesktop: window.innerWidth >= 1024 }" x-init="
    ...
    <div class="h-[72px] flex items-center justify-between px-4 border-b border-gray-100 shrink-0">
        <div class="flex items-center gap-2 min-w-0" :class="$store.sidebar.collapsed ? 'lg:justify-center lg:w-full' : ''">
            {{-- Tombol Toggle Desktop (Panah kiri-kanan) --}}
            {{-- Ini SATU-SATUNYA aksi yang boleh memicu submenu aktif otomatis terbuka (saat expand). --}}
            <button @click="$store.sidebar.toggleCollapsed()" title="Buka/Tutup Sidebar"
        ...
        {{-- Tombol Tutup Mobile --}}
        <button @click="$store.sidebar.mobileOpen = false" class="lg:hidden text-gray-400 hover:text-gray-600 shrink-0">
            <i data-lucide="x" class="w-5 h-5"></i>
        </button>
    </div>

    {{-- AREA MENU (Bisa di-scroll jika panjang) --}}
    <div class="flex-1 overflow-y-auto overflow-x-hidden py-6 px-3 flex flex-col gap-6 no-scrollbar"
        :class="($store.sidebar.collapsed && isDesktop) ? 'items-center' : ''">

        {{-- MENU 1: DASHBOARD (Berdiri Sendiri) --}}
        <div class="w-full">
            <div class="flex flex-col gap-1 w-full" :class="($store.sidebar.collapsed && isDesktop) ? 'items-center' : ''">
                {{-- Dashboard --}}
        ...
        {{-- MENU 2: KATALOG LAYANAN (DROPDOWN) --}}
        @php
            $katalogActive = request()->routeIs('service-categories.*', 'service-types.*');
        @endphp
        <div class="w-full" x-data="{
                isActive: {{ $katalogActive ? 'true' : 'false' }},
                open: false,
                flyoutStyle: ''
             }" x-init="
                // Saat halaman pertama kali dimuat / berpindah halaman:
                // - Kalau sidebar SEDANG TERBUKA (tidak collapsed) dan menu ini aktif -> submenu inline langsung terbuka.
                // - Kalau sidebar SEDANG COLLAPSED -> jangan buka apa pun (termasuk flyout), walau menu ini aktif.
                //   Flyout hanya boleh terbuka lewat klik langsung pada ikon menu.
                open = isActive && !($store.sidebar.collapsed && isDesktop);

                // Saat status collapsed BERUBAH (dipicu klik panah toggle di atas):
                // - Berubah jadi collapsed -> tutup semua (submenu inline maupun flyout).
                // - Berubah jadi expanded -> baru di sinilah submenu otomatis terbuka kalau menu ini aktif.
                $watch('$store.sidebar.collapsed', (val) => {
                    if (!isDesktop) return;
                    open = val ? false : isActive;
                });
                $watch('$store.sidebar.mobileOpen', (val) => {
                ...
                $watch('isDesktop', () => {
                    if (!isDesktop) {
                        ...
                        open = $store.sidebar.collapsed ? false : isActive;
                    }
                });
             " @click.outside="if ($store.sidebar.collapsed && isDesktop) { open = false; }">
            <div class="flex flex-col gap-1 w-full" :class="($store.sidebar.collapsed && isDesktop) ? 'items-center' : ''">
                {{-- Induk Katalog --}}
                <button @click="
                            if ($store.sidebar.collapsed && isDesktop) {
                                open = !open;
                                if (open) {
                                    $nextTick(() => {
                    ...
                    <div class="flex items-center"
                        ...
                        <i data-lucide="layers"
                            class="w-5 h-5 shrink-0 {{ $katalogActive ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        <span x-show="!($store.sidebar.collapsed && isDesktop)" class="whitespace-nowrap">Katalog Layanan</span>
                    </div>
                    <i x-show="!($store.sidebar.collapsed && isDesktop)" data-lucide="chevron-down"
                        class="w-4 h-4 transition-transform duration-200" :class="open ? 'rotate-180' : ''"></i>
                </button>
                {{-- Sub-Menu Inline --}}
                <div x-show="open && !($store.sidebar.collapsed && isDesktop)"
                    ...
                    <div class="pl-9 pr-2 py-1 flex flex-col gap-1 mt-1">
                        <a href="{{ route('service-categories.index') }}"
                           ...
                           {{ request()->routeIs('service-categories.*') ? 'text-blue-600 font-semibold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                            <div
                                class="w-1.5 h-1.5 rounded-full shrink-0 {{ request()->routeIs('service-categories.*') ? 'bg-blue-600' : 'bg-gray-300' }}">
                            </div>
                            Kategori & Portofolio
                        </a>
                        <a href="{{ route('service-types.index') }}"
                           ...
                           {{ request()->routeIs('service-types.*') ? 'text-blue-600 font-semibold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                            <div
                                class="w-1.5 h-1.5 rounded-full shrink-0 {{ request()->routeIs('service-types.*') ? 'bg-blue-600' : 'bg-gray-300' }}">
                            </div>
                            Daftar Paket
                        </a>
                    </div>
                </div>
                {{-- Flyout Popup Desktop (hanya via klik langsung ikon ini) --}}
                <div x-show="open && ($store.sidebar.collapsed && isDesktop)"
                    ...
                    <a href="{{ route('service-categories.index') }}" @click="open = false"
                    ...
                    <a href="{{ route('service-types.index') }}" @click="open = false"
                        class="flex items-center gap-3 px-4 py-2.5 text-[13px] transition
        ...
        @php
            $operasionalActive = request()->routeIs('bookings.listPage', 'bookings.calendar', 'transactions.index', 'financial.index', 'workboard.index');
        @endphp
        <div class="w-full" x-data="{
                isActive: {{ $operasionalActive ? 'true' : 'false' }},
                open: false,
                flyoutStyle: ''
             }" x-init="
                open = isActive && !($store.sidebar.collapsed && isDesktop);
            ...
            <div class="flex flex-col gap-1 w-full" :class="($store.sidebar.collapsed && isDesktop) ? 'items-center' : ''">
                ...
                <button @click="
                        ...
                        " title="Operasional"
                    class="flex items-center px-3 py-2.5 rounded-xl font-medium text-[13px] transition w-full group focus:outline-none
                        {{ $operasionalActive ? 'bg-blue-50/50 text-blue-600 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}"
                    :class="($store.sidebar.collapsed && isDesktop) ? 'justify-center w-11 h-11 px-0' : 'justify-between'">
                    <div class="flex items-center"
                        :class="($store.sidebar.collapsed && isDesktop) ? 'justify-center' : 'gap-3'">
                        <i data-lucide="briefcase"
                            class="w-5 h-5 shrink-0 {{ $operasionalActive ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600' }}"></i>
                        <span x-show="!($store.sidebar.collapsed && isDesktop)" class="whitespace-nowrap">Operasional</span>
                    </div>
                    <i x-show="!($store.sidebar.collapsed && isDesktop)" data-lucide="chevron-down"
                        class="w-4 h-4 transition-transform duration-200" :class="open ? 'rotate-180' : ''"></i>
                </button>
                    ...
                    <div class="pl-9 pr-2 py-1 flex flex-col gap-1 mt-1">
                        <a href="{{ route('bookings.listPage') }}"
                           ...
                           {{ request()->routeIs('bookings.listPage') ? 'text-blue-600 font-semibold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-1.5 h-1.5 rounded-full shrink-0 {{ request()->routeIs('bookings.listPage') ? 'bg-blue-600' : 'bg-gray-300' }}">
                                </div>
                                Booking
                            </div>
                            @if($sidebarPendingBookingCount > 0)
                                <span
                                    class="text-[10px] font-bold px-2 py-0.5 rounded-full min-w-[20px] text-center bg-amber-100 text-amber-700">
                        ...
                        <a href="{{ route('transactions.index') }}"
                           ...
                           {{ request()->routeIs('transactions.index') ? 'text-blue-600 font-semibold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-1.5 h-1.5 rounded-full shrink-0 {{ request()->routeIs('transactions.index') ? 'bg-blue-600' : 'bg-gray-300' }}">
                                </div>
                        ...
                        <a href="{{ route('financial.index') }}"
                           ...
                           {{ request()->routeIs('financial.index') ? 'text-blue-600 font-semibold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                            <div
                                class="w-1.5 h-1.5 rounded-full shrink-0 {{ request()->routeIs('financial.index') ? 'bg-blue-600' : 'bg-gray-300' }}">
                            </div>
                            Laporan Keuangan
                        </a>
                    </div>
                ...
                <div x-show="open && ($store.sidebar.collapsed && isDesktop)"
                    ...
                    x-transition:leave-end="opacity-0" :style="flyoutStyle"
                    class="fixed w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-[100]" x-cloak>
                    <div class="px-4 py-2 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Operasional
                    </div>
                    <a href="{{ route('bookings.listPage') }}" @click="open = false"
    ...
    <div class="p-3 border-t border-gray-100 bg-gray-50/50 shrink-0 w-full"
        ...
        <div class="flex items-center gap-3 px-2 py-2 mb-1"
            ...
            <div
                class="w-9 h-9 rounded-full bg-blue-100 text-blue-600 font-bold flex items-center justify-center shrink-0 uppercase">
                {{ substr(Auth::user()->email, 0, 1) }}
            </div>
            <div x-show="!($store.sidebar.collapsed && isDesktop)" class="min-w-0">
        ...
        <form method="POST" action="{{ route('logout') }}" class="w-full" data-turbo="false">
            ...
            <button type="submit" title="Keluar"
                ...
                :class="($store.sidebar.collapsed && isDesktop) ? 'justify-center w-11 h-11 mx-auto px-0' : 'px-4 py-2.5 gap-2 justify-center'">
                <i data-lucide="log-out" class="w-4 h-4 shrink-0"></i>
                <span x-show="!($store.sidebar.collapsed && isDesktop)">Keluar</span>
            </button>
        </form>
    </div>
</aside>