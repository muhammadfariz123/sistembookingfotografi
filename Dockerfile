...
# Perintah otomatis saat server menyala (Menunggu koneksi DB siap sebelum migrasi, tanpa reset data)
CMD bash -c "for i in {1..30}; do php artisan migrate:status > /dev/null 2>&1 && break || sleep 1; done && (php artisan migrate --force ; php artisan db:seed --class=AdminSeeder --force || true) && php -S 0.0.0.0:$PORT -t public"